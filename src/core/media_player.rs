...
use egui::{
    vec2, Align2, Color32, ColorImage, Image, Rect, Response, Rounding, Sense, Spinner,
...
impl Player {
    ...
    pub fn render_controls(&mut self, ui: &mut Ui, frame_response: &Response) {
        ...
        if is_audio_cyclable {
            ...
            if stream_anim_frac > 0. && is_audio_cyclable {
                draw_row(Type::Audio);
            }
...
pub trait Streamer: Send {
    ...
    fn seek(&mut self, seek_frac: f32) {
        ...
        if !seek_completed {
            ...
            if self.input_context().seek(target_ts, ..target_ts).is_err() {
